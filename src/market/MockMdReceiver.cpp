...
void MockMdReceiver::threadLoop() {
  ...
  while (running_.load(std::memory_order_acquire)) {
    ...
    // T1 Timestamp: Record High-Precision local time exactly before pushing.
    // This is the penetration latency origin.
    ...
    // If queue is full, this will spin (or just drop the tick, we drop here to
    // avoid blocking snapshot) Spinning is also valid; we just try once. If
    // queue fails, mock data is discarded (representing buffer overrun).
    if (!queue_.push(tick)) {
      // Yield if full
